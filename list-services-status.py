...
# It is preserved here to check against the live services.
SERVICE_DATABASE = {
  # Accessibility
...
def get_all_services(console: Console) -> Set[str]:
  """
  Discovers all launchd services, both system-level daemons and user-level agents.
  """
  all_services = set()
  console.print("[cyan]Discovering system daemons (requires sudo)...[/cyan]")
  ...
  try:
    ...
    for line in daemons_raw.strip().split("\n")[1:]:
      parts = line.split()
      if len(parts) >= 3 and (
        "com.apple" in parts[2]
        or "com.vix.cron" in parts[2]
        or "org.cups.cupsd" in parts[2]
      ):
        all_services.add(parts[2])
  except subprocess.CalledProcessError as e:
    console.print(f"[bold red]Error getting system daemons: {e}[/bold red]")
  except FileNotFoundError:
    console.print("[bold red]Error: 'launchctl' command not found.[/bold red]")

  console.print("[cyan]Discovering user agents...[/cyan]")
  # Get user agents
  try:
    ...
    for line in agents_raw.strip().split("\n")[1:]:
      parts = line.split()
      if len(parts) >= 3 and ("com.apple" in parts[2]):
        all_services.add(parts[2])
  except subprocess.CalledProcessError as e:
    console.print(f"[bold red]Error getting user agents: {e}[/bold red]")
  except FileNotFoundError:
    console.print("[bold red]Error: 'launchctl' command not found.[/bold red]")
...
def audit_services(console: Console, services: Set[str]):
  """
  Audits the provided set of services against the SERVICE_DATABASE and prints a report.
  """
  table = Table(title="macOS Service Database Audit")
  table.add_column("Service Name", style="cyan", no_wrap=True)
  table.add_column("Details Status", justify="center")

  found_count = 0
  missing_count = 0
  
  # Also include services that are in the database but might not be running on the system
  all_service_keys = sorted(list(services.union(set(SERVICE_DATABASE.keys()))))


  for service_name in all_service_keys:
    if service_name in SERVICE_DATABASE:
      table.add_row(service_name, "[green]✅ Found[/green]")
      found_count += 1
    else:
      table.add_row(service_name, "[red]❌ Missing[/red]")
      missing_count += 1

  console.print(table)
  console.print("\n--- Audit Summary ---")
  console.print(f"[green]Documented Services: {found_count}[/green]")
  console.print(f"[red]Undocumented Services: {missing_count}[/red]")
  console.print(f"[bold]Total Services Checked: {len(all_service_keys)}[/bold]")


if __name__ == "__main__":
  if sys.platform != "darwin":
    print("This script is intended to be run on macOS only.")
    sys.exit(1)

  console = Console()
  all_found_services = get_all_services(console)

  if not all_found_services:
    console.print("[bold red]Could not retrieve any services. Exiting.[/bold red]")
    sys.exit(1)

  audit_services(console, all_found_services)
